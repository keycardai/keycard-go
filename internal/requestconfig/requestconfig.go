...
func getDefaultHeaders() map[string]string {
	return map[string]string{
		"User-Agent": fmt.Sprintf("KeycardAPI/Go %s", internal.PackageVersion),
	}
}
...
func NewRequestConfig(ctx context.Context, method string, u string, body any, dst any, opts ...RequestOption) (*RequestConfig, error) {
	...
	cfg.Security = Security{
		SvcOrgManagementBasicAuth: true,
		VaultBearerAuth:           true,
	}
	err = cfg.Apply(opts...)
	if err != nil {
		return nil, err
...
type Security struct {
	SvcOrgManagementBasicAuth bool
	VaultBearerAuth           bool
}
...
// WithSvcOrgManagementBasicAuthSecurity() should only be used within a method, not
// provided to at the client-level.
func WithSvcOrgManagementBasicAuthSecurity() RequestOption {
	return RequestOptionFunc(func(r *RequestConfig) error {
		r.Security = Security{
			SvcOrgManagementBasicAuth: true,
			VaultBearerAuth:           false,
		}
		return nil
	})
}

// WithVaultBearerAuthSecurity() should only be used within a method, not provided
// to at the client-level.
func WithVaultBearerAuthSecurity() RequestOption {
	return RequestOptionFunc(func(r *RequestConfig) error {
		r.Security = Security{
			SvcOrgManagementBasicAuth: false,
			VaultBearerAuth:           true,
		}
		return nil
	})
...
func ApplySecurity(r RequestConfig) {
	if r.Security.SvcOrgManagementBasicAuth && r.Username != "" && r.Password != "" && r.Request.Header.Get("Authorization") == "" {
		r.Request.Header.Set("authorization", fmt.Sprintf("Basic %s", base64.StdEncoding.EncodeToString([]byte(r.Username+":"+r.Password))))
	}

	if r.Security.VaultBearerAuth && r.APIKey != "" && r.Request.Header.Get("Authorization") == "" {
